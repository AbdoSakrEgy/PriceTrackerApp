...
class SocketioServices {
  ...
  sendMessage = (socket) => {
    socket.on("sendMessage", async (arg, callback) => {
      try {
        const { sendTo, content } =
          chat_validation_1.chatMessageSchema.parse(arg);
        ...
        if (!chat) {
          throw new Errors_1.ApplicationException("Chat not found", 404);
        }
  ...
  joinRoom = (socket) => {
    socket.on("join_room", async (arg, callback) => {
      try {
        ...
        if (!group) {
          throw new Errors_1.ApplicationException("Group not found", 404);
        }
        socket.join(groupId);
      } catch (err) {
    ...
    socket.on("sendGroupMessage", async (arg, callback) => {
      try {
        const { content, groupId } =
          chat_validation_1.chatGroupMessageSchema.parse(arg);
        const createdBy = socket.user?._id;
        const group = await this.chatRepo.findOne({
          filter: {
            _id: groupId,

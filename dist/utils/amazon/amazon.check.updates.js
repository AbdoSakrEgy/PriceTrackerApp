...
exports.amazonCheckUpdates = amazonCheckUpdates;
const amazon_model_1 = require("../../modules/amazon/amazon.model");
const amazon_extractor_1 = require("./amazon.extractor");
async function amazonCheckUpdates() {
    const updatedProductsArr = [];
    const allProducts = await amazon_model_1.AmazonModel.find();
    for (const product of allProducts) {
        // step: check product updates
        const currentProductData = await (0, amazon_extractor_1.amazonExtractor)(product.url);
        ...
        const updatedProduct = await amazon_model_1.AmazonModel.findOneAndUpdate({ url: product.url }, { $push: { updateLog: newProductVersion } }, { new: true });

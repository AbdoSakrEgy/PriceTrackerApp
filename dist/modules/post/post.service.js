...
class PostServices {
  ...
  createPost = async (req, res, next) => {
    ...
    const post = await this.postRepo.create({
      data: {
        ...req.body,
        // not working, Because you send data using form/data which not parsing data
        // content,
        // avilableFor,
        // isCommentsAllowed,
        // tags,
    ...
    tags?.map(async (tag) => {
      ...
      if (!taggedUser?.email || !taggedUser?.firstName) {
        throw new Errors_1.ApplicationException("Tagged user not found", 404);
      }
  ...
  updatePost = async (req, res, next) => {
            ...
            $setUnion: [
              {
                $setDifference: [
                  "$tags",
                  removedTags.map((id) =>
                    mongoose_1.Types.ObjectId.createFromHexString(id)
    ...
    //     throw new ApplicationException ("Some tags users not found", 404);
    //   }
    //   updatedPost = await this.postRepo.findOneAndUpdate({
    //     filter: { _id: postId },

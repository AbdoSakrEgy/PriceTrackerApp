...
class CommentServices {
  ...
  addComment = async (req, res, next) => {
    ...
    if (!post) {
      throw new Errors_1.ApplicationException("Post not found", 404);
    }
    // step: check is comments allowed
    if (!post.isCommentsAllowed) {
      throw new Errors_1.ApplicationException("Comments not allowed", 400);
    }
    ...
    if (!post.createdBy.equals(user._id)) {
      ...
      if (
        post.avilableFor == post_module_types_1.PostAvilableForEnum.FRIENDS &&
      ...
      if (
        post.avilableFor == post_module_types_1.PostAvilableForEnum.PRIVATE &&
  ...
  updateComment = async (req, res, next) => {
    ...
    if (!comment) {
      throw new Errors_1.ApplicationException("Comment not found", 404);
    }
  ...
  deleteComment = async (req, res, next) => {
    ...
    const commentWithReplies = await (0,
    comment_searcher_1.getAllChildCommentIds)(
      commentId,
  ...
  getComment = async (req, res, next) => {
    ...
    // step: get comments
    const childCommentIds = await (0, comment_searcher_1.getAllChildComments)(
      commentId,
      comment_model_1.CommentModel
    );
    return (0, successHandler_1.successHandler)({
      res,
      result: { comment, childCommentIds },
    });
  };
}
